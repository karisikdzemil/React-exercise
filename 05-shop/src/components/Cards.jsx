    ...
    return (
        <ul className="w-[100%] min-h-[90vh] px-16 flex flex-wrap gap-[20px] justify-center p-5">
            {shopItems.map((el, i) => (
                 ...
                 <p ref={productPrice} className="">Cena: {el.price} RSD</p>
                 <button onClick={addToCart} className="w-30 h-10 mt-2 border-2 border-gray-700 text-gray-700 hover:bg-red-500 rounded-md">Add to cart</button>
             </li>
            ))}
        </ul>
    )
